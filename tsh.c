...
eval(char *cmdline)
{
    ...
    stdo = dup(STDOUT_FILENO); /* record standard input and output */
    int input_fd = ST_INFILE;
    int output_fd = ST_OUTFILE;
    if(tok.infile != NULL){
        input_fd = open(tok.infile, O_RDONLY, 0);
        dup2(input_fd, STDIN_FILENO);
    }
    if(tok.outfile != NULL){
        output_fd = open(tok.outfile, O_RDWR, 0);
    ...
    sigaddset(&mask, SIGTSTP); /* set mask */
    ...
    if(tok.builtins == BUILTIN_QUIT){
        ...
        if(tok.argv[1][0] == '%'){
            ...
            tmppid = atoi(tok.argv[1]);
            job = getjobpid(job_list, tmppid);
            if(job == NULL){
                sio_puts("(");
                sio_putl(tmppid);
    ...
    else
    {
        ...
        /* parent process */
        addjob(job_list, pid, bg ? BG:FG, cmdline); /* add the child to the job list */
